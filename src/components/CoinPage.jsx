 ...
 *  CoinDetailsPage
 *  Props:
 *    • coinDetails – full object returned by the API
 *    • currentCurrency – { name: "usd", symbol: "$" } (or similar)
 *    • period – selected time‑range value (1, 7, 30 …)
 *    • setPeriod – setter from useState
 *    • chartData – data already shaped for <AreaChart />
 *  ---------------------------------------------------------------- */
...
return (
  ...
  {/* 24h Change */}
  <div className="flex-1 bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20">
    <div className="flex justify-between items-center">
  ...
  <div className="flex-1 bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20">
    <div className="flex justify-between items-center">
      ...
      <span className="text-lg md:text-xl text-cyan-400">
        {currentCurrency.symbol}
        {coinDetails.market_data?.total_volume?.[currentCurrency.name]
  ...
  {/* Example 3rd Card – You can change this to something else like Market Cap or Circulating Supply */}
  <div className="flex-1 bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20">
    <div className="flex justify-between items-center">
      <span className="text-sm md:text-base text-emerald-400/90">Market Cap</span>
      <span className="text-lg md:text-xl text-cyan-400">
        {currentCurrency.symbol}
        {coinDetails.market_data?.market_cap?.[currentCurrency.name]
          ? coinDetails.market_data.market_cap[currentCurrency.name].toLocaleString()
          : "--"}
      </span>
    </div>
